...
Mobile audio/menu correction (2026-09-23): the ambient thunder loop now starts and remains at 0.001 volume on coarse/mobile pointers (0.012 on desktop), keeping it effectively muted beneath the voice without a gesture-triggered fade-in. All encounter errors now remain on the stage warning and close any open dialog; the menu is user-controlled and never opens just to report an error. This prevents mobile conversation failures from interrupting the visitor with a menu popup.
